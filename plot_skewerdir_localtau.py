#!/usr/bin/env python3
"""
This script will plot the local optical depth distribution as a function
...
Usage for directory named skewers:
    $ python3 plot_skewerdir_localtau.py skewers/ -v 
"""
import argparse
...
def create_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Plot the distribution of local optical depths")
...
def main():
    '''
    Plot the local optical depth as a function of redshift
    '''
    ...
    hist_local_ltau = np.zeros((tau_nbins-1, nOutputs))
    tau_local_mean_arr = np.zeros(nOutputs, dtype=precision)
    tau_local_18perc_arr = np.zeros(nOutputs, dtype=precision)
    tau_local_50perc_arr = np.zeros(nOutputs, dtype=precision)
    tau_local_84perc_arr = np.zeros(nOutputs, dtype=precision)
    ...
    for fPath in skewer_dirPath.iterdir():
        ...
        # grab localective optical depth
        ...
        tau_local_all = np.zeros(totnCells, dtype=precision)
        ...
        tau_local_mean_arr[n] = np.mean(tau_local_all)
        tau_local_18perc_arr[n] = np.percentile(tau_local_all, 18)
        tau_local_50perc_arr[n] = np.percentile(tau_local_all, 50)
        tau_local_84perc_arr[n] = np.percentile(tau_local_all, 84)

        # histogram it !
        l_tau_hist_all, _ = np.histogram(np.log10(tau_local_all), bins=l_tau_bins)

        # place onto global array
        hist_local_ltau[:,n] += l_tau_hist_all

        # normalize at each redshift bin
        hist_local_ltau[:,n] = hist_local_ltau[:,n] / totnCells
        n += 1
    
    indices_redshiftsort = np.argsort(redshift_bins)
    redshift_bins_sorted = redshift_bins[indices_redshiftsort]
    hist_local_ltau_sorted = hist_local_ltau[:, indices_redshiftsort[1:]] # first index will be zero
    tau_local_mean_arr_sorted = tau_local_mean_arr[indices_redshiftsort[1:]]
    tau_local_18perc_arr_sorted = tau_local_18perc_arr[indices_redshiftsort[1:]]
    tau_local_50perc_arr_sorted = tau_local_50perc_arr[indices_redshiftsort[1:]]
    tau_local_84perc_arr_sorted = tau_local_84perc_arr[indices_redshiftsort[1:]]

    redshift_center_sorted = (redshift_bins_sorted[1:] + redshift_bins_sorted[:-1]) / 2.

    yaxis_label = r'$\tau_{\rm{local}}$'
    ...
    im0 = ax.pcolormesh(redshift_bins_sorted, 10**(l_tau_bins), 
                        np.log10(hist_local_ltau_sorted))
    _ = ax.plot(redshift_center_sorted, tau_local_mean_arr_sorted, ls='--', 
                label=r'$\rm{Mean}$', marker='.', markersize=10, zorder=3)
    _ = ax.plot(redshift_center_sorted, tau_local_18perc_arr_sorted, ls='-', 
                label=r'$18-50-84 \%$', c='k', marker='.', markersize=10)
    _ = ax.plot(redshift_center_sorted, tau_local_50perc_arr_sorted, ls='-', 
                c='k', marker='.', markersize=10)
    _ = ax.plot(redshift_center_sorted, tau_local_84perc_arr_sorted, ls='-', 
                c='k', marker='.', markersize=10)
    ...
    cbar_str = r"$\log_{10} \rm{P}(\tau_{\rm{local}} | z)$"
