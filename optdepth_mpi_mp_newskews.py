...
class ChollaSkewers_iHead:
    ...
    '''
    def __init__(self, n_i, n_j, n_k, n_skews, skew_key):
        self.n_i = n_i
        self.n_j = n_j
        self.n_k = n_k


class ChollaSkewers_i:
    '''
    Cholla Skewers
    ...
    def __init__(self, ChollaSkewersiHead, fPath, comm):
        self.SkewersiHead = ChollaSkewersiHead
    ...
    def set_keys(self):
        ...
        with h5py.File(self.fPath, 'r') as fObj:
            self.allkeys = set(fObj[self.SkewersiHead.skew_key].keys())
            for key in self.allkeys:
                if fObj[self.SkewersiHead.skew_key].get(key).ndim == 1:
                    keys_1D.append(key)
                if fObj[self.SkewersiHead.skew_key].get(key).ndim == 2:
                    keys_2D.append(key)
    ...
    def get_skeweralldata(self, key, dtype=np.float32):
        ...
        if key in self.keys_1D:
            arr = np.zeros((self.SkewersiHead.n_skews), dtype=dtype)
            with h5py.File(self.fPath, 'r') as fObj:
                arr[:] = fObj[self.SkewersiHead.skew_key].get(key)[:]
        elif key in self.keys_2D:
            arr = np.zeros((self.SkewersiHead.n_skews, self.SkewersiHead.n_i), dtype=dtype)
            with h5py.File(self.fPath, 'r') as fObj:
                arr[:,:] = fObj[self.SkewersiHead.skew_key].get(key)[:, :]
...
class ChollaSkewers:
    ...
    def __init__(self, fPath, axis, comm):
    ...
    def set_gridinfo(self, datalength_str='HI_density'):
        ...
        Returns:
            ...
        '''
        with h5py.File(self.SkewersfPath, 'r', driver='mpio', comm=self.comm) as fObj:
    ...
    def print_gridinfo(self):
        ...
        print(f"--- {rank_idstr} : We have {self.nSkewers} total number of skewers ---")
        print(f"--- {rank_idstr} : Line of sight is {self.nlos} cells long with total length of {self.L_los_h_kpc:.4f} h-1 kpc---")
        print(f"--- {rank_idstr} : Cell width along LOS are {self.dlos_h_kpc:.4f} h-1 kpc")
        print(f"--- {rank_idstr} : Hubble flow along a cell is {self.dvHubble_los:.4f} km / s")
    ...
    def print_cosmoinfo(self):
        ...
        rank = self.comm.Get_rank()
        rank_idstr = f"Rank {rank:.0f}"
        print(f"--- {rank_idstr} : studying skewers at z={self.current_z:.3f} / a={self.current_a:.3f} ---")
        print(f"--- {rank_idstr} : energy density info ---")
        print(f"--- {rank_idstr} : \t OmegaM = {self.Omega_M:.4f} ---")
        print(f"--- {rank_idstr} : \t OmegaDE = {self.Omega_L:.4f} ---")
        print(f"--- {rank_idstr} : \t OmegaR = {self.Omega_R:.4f} ---")
        print(f"--- {rank_idstr} : \t OmegaK = {self.Omega_K:.4f} ---")
        print(f"--- {rank_idstr} : \t w0, wa = {self.w0:.4f}, {self.wa:.4f} ---")
        print(f"--- {rank_idstr} : H0 = {self.H0:.4f} km / s / Mpc")
        print(f"--- {rank_idstr} : H(z={self.current_z}) = {self.get_currH():.4f} km / s / Mpc")
...
def init_taucalc(Skewers, comm, restart = False, verbose=False, nopec=False):
    '''
    ...
    with h5py.File(Skewers.SkewersfPath, 'r+', driver='mpio', comm=comm) as fObj:
        ...
        if verbose:
            print(f'--- {rank_idstr} : \t...initializing optical depth calculations for file {Skewers.SkewersfPath} ---')
        ...
        if verbose:
            print(f"--- {rank_idstr} : \t\t...initializing arrays and attributes ---")
...
def taucalc(Skewers, skewCosmoCalc, comm, precision=np.float64, verbose=False, nopec=False, nprocs=2):
    '''
    ...
    with h5py.File(Skewers.SkewersfPath, 'r+', driver='mpio', comm=comm) as fObj:
        ...
        if verbose:
            print(f"--- {rank_idstr} : Starting calculations at {100 * curr_progress:.2f} % complete along ---")
        ...
        skewerIDs_tmp_mask = np.arange(skewerIDs_2load.size)
        skewerIDs_2load_rank = np.argwhere( (skewerIDs_tmp_mask % size) == rank).flatten()
        nSkewers_2load_rank = skewerIDs_2load_rank.size

        if nopec:
            full_data = np.zeros( (nSkewers_2load_rank, 2, Skewers.nlos) , dtype=precision)
            full_data[ :, 0, : ] = fObj.get('HI_density')[skewerIDs_2load_rank]
            full_data[ :, 1, : ] = fObj.get('temperature')[skewerIDs_2load_rank]
        else:
            full_data = np.zeros( (nSkewers_2load_rank, 3, Skewers.nlos) , dtype=precision)
            full_data[ :, 0, : ] = fObj.get('HI_density')[skewerIDs_2load_rank]
            full_data[ :, 1, : ] = fObj.get('velocity')[skewerIDs_2load_rank]
            full_data[ :, 2, : ] = fObj.get('temperature')[skewerIDs_2load_rank]

          
        with Pool(processes=nprocs) as pool:
            chunksize = 32 # chosen from experimentation
            if nopec:
            ...
            for skewerID_local, taus_skew in enumerate(res):
                ...
                skewerID = skewerIDs_2load_rank[skewerID_local]
    ...
    if verbose:
        print(f"--- {rank_idstr} : Effective optical depth calculation completed ---")
...
def main():
    ...
    if args.verbose and rank == 0:
        print(f"{rank_idstr} : Using {size:.0f} ranks and {args.nprocs:.0f} processes!")
    ...
    if args.verbose:
        ...
        ix_str = ['x', 'y', 'z']
        print(f"--- {rank_idstr} : skewers drawn along {ix_str[args.axis]} axis ---")

    # create ChollaSkewers object
    Skewers = ChollaSkewers(skewer_fPath, args.axis, comm)

    if args.verbose and not rank:
        print(f"--- {rank_idstr} : Skewer object created ---")
        Skewers.print_cosmoinfo()
        Skewers.print_gridinfo()

    # add progress attribute, boolean mask for whether tau is calculated, and tau itself
    t_init_start = MPI.Wtime()
    init_taucalc(Skewers, comm, restart=args.restart, verbose=args.verbose, nopec=args.peculiarless)
    ...
    if args.verbose:
        print(f"--- {rank_idstr} : Cosmo Head object created ---")

    skewCosmoCalc = ChollaSkewerCosmoCalculator(Skewers.current_a, chCosmoHead, 
                                                  Skewers.nlos, Skewers.dlos_h_kpc, precision)

    if args.verbose:
        print(f"--- {rank_idstr} : Skewer Cosmo Calculator object created ---")

    t_start = MPI.Wtime()
    taucalc(Skewers, skewCosmoCalc, comm, precision, args.verbose, args.peculiarless, args.nprocs)
    t_end = MPI.Wtime()
