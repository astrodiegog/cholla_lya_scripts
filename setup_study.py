...
def create_powspecplot_slurm(powspecdiff_pyscript_fPath, powspecdiff_slurm_fPath, dlogk, skewers_DirPath, analysis_DirPath, scriptlog_DirPath, plots_DirPath, studyName, nOutputs, verbose=False):
    '''
    Create powspecdiff.slurm file that will complete the optical depth calculations

    Args:
        powspecdiff_pyscript_fPath (Path): path to power spectra difference plotting script
        powspecdiff_slurm_fPath (Path): path to place power spectra difference slurm file
    ...
    bash_header = f'''#!/bin/bash
#SBATCH --job-name={studyName}-powspecdiff      # Job name
#SBATCH --partition=cpuq # Partition name
#SBATCH --account=cpuq   # Account name
#SBATCH --exclusive             # Exclusive use of the node
#SBATCH --ntasks=1              # Number of MPI ranks
...
#SBATCH --ntasks-per-node=1     # How many tasks on each node
#SBATCH --time=24:00:00         # Time limit (hh:mm:ss)
#SBATCH --output={scriptlog_DirPath}/powspecdiff_%j.log     # Standard output and error log
#SBATCH --mail-type=FAIL
#SBATCH --mail-type=BEGIN
#SBATCH --mail-type=END
#SBATCH --mail-user=digarza@ucsc.edu
...
'''
    ...
    bash_scriptnfiles = f'''
scriptPath="{powspecdiff_pyscript_fPath}"
...
outDir=$plotsDir"/powspecdiff_plots"
fname="PowerSpectraDiff_"$SLURM_ARRAY_TASK_ID".png"

outDirLog=$plotsDir"/powspecdiff_log_plots"
fnameLog="PowerSpectraLogDiff_"$SLURM_ARRAY_TASK_ID".png"
...
if [ ! -d "$outDirLog" ]; then
  echo "$outDirLog does not exist. Creating it"
  mkdir $outDirLog
fi
'''
...
srun -N 1 -n 1 -c 1 --cpu-bind=cores --exclusive --partition=cpuq --account=cpuq python3 $scriptPath $skewerFile $analysisFile $dlogk -v -o $outDir -f $fname

srun -N 1 -n 1 -c 1 --cpu-bind=cores --exclusive --partition=cpuq --account=cpuq python3 $scriptPath $skewerFile $analysisFile $dlogk -v -l -o $outDirLog -f $fnameLog
'''

    # combine text and write to file
    bash_txt = bash_header + bash_load + bash_scriptnfiles + bash_outdirsnfnames + bash_srun
    with powspecdiff_slurm_fPath.open('w') as slurmfile:
        slurmfile.write(bash_txt)

    if verbose:
        print(f"\t --- Woo hoo! We now have power spectra difference plotting slurm file : {powspecdiff_slurm_fPath} ---")
