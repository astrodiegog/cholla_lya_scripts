#!/usr/bin/env python3
"""
This script will plot the optical depth corresponding to the mean flux in a 
    distribution as a function of redshift. It will take in a skewer directory 
    and a list of output strings and make a 2D histogram.

Usage for directory named skewers:
    $ python3 plot_skewerdir_tau_meanF_local.py skewers/ -v 
"""
import argparse
...
def create_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Plot the distribution of optical depths of mean flux")
...
def main():
    ...
    if args.fname:
        ...
        fName = f'tau_meanF_localdistr.png'
    ...
    for fPath in skewer_dirPath.iterdir():
        ...
        nCells_x = tau_local_x.size
        nCells_y = tau_local_y.size
        nCells_z = tau_local_z.size
        totnCells = nCells_x + nCells_y + nCells_z

        # place all fluxes into one array
        fluxes_all = np.zeros(totnCells, dtype=precision)
        fluxes_all[ : (nCells_x)] = fluxes_x.flatten()
        fluxes_all[ (nCells_x) : (nCells_x + nCells_y)] = fluxes_y.flatten()
        fluxes_all[ (nCells_x + nCells_y) : ] = fluxes_z.flatten()

        # calculate mean, 16-50-84 percentiles
        tau_meanF_mean_arr[n] = -np.log( np.mean(fluxes_all) )
        tau_meanF_18perc_arr[n] = -np.log( np.percentile(fluxes_all, 18) )
        tau_meanF_50perc_arr[n] = -np.log( np.percentile(fluxes_all, 50) )
        tau_meanF_84perc_arr[n] = -np.log( np.percentile(fluxes_all, 84) )

        # histogram it !
        l_tau_hist_all, _ = np.histogram(np.log10(- np.log(fluxes_all) ), bins=l_tau_bins)

        # place onto global array
        hist_meanF_ltau[:,n] += l_tau_hist_all

        # normalize at each redshift bin
        hist_meanF_ltau[:,n] = hist_meanF_ltau[:,n] / totnCells
        n += 1
    ...
    cbar_str = r"$\log_{10} \rm{P}(-\ln (F_{\rm{local}}) | z)$"
