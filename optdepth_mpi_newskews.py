...
def init_taucalc(Skewers, comm, restart = False, verbose=False, nopec=False):
    ...
    with h5py.File(Skewers.SkewersfPath, 'r+', driver='mpio', comm=comm) as fObj:
        ...
        Skewers_lst = [Skewers.get_skewersx_obj(),
                        Skewers.get_skewersy_obj(),
                        Skewers.get_skewersz_obj()]

        # add progress attribute, boolean mask for whether tau is calculated, and tau itself
        for i, Skewers_i in enumerate(Skewers_lst):

            if verbose:
                print(f"--- {rank_idstr} : \t\t...initializing arrays and attributes along axis {i:.0f} ---")
            skew_key = Skewers_i.SkewersiHead.skew_key

            taucalc_bool = np.zeros(Skewers_i.SkewersiHead.n_skews, dtype=bool)
            taucalc_eff = np.zeros(Skewers_i.SkewersiHead.n_skews, dtype=np.float64)

            taucalc_local = np.zeros((Skewers_i.SkewersiHead.n_skews, Skewers_i.SkewersiHead.n_i),
                                        dtype=np.float64)
...
def taucalc(Skewers_i, skewCosmoCalc, comm, precision=np.float64, verbose=False, nopec=False):
    '''
    Calculate the effective optical depth for each skewer along an axis

    Args:
        Skewers_i (ChollaSkewers_i): holds all skewer info along an axis
    ...
    with h5py.File(Skewers_i.fPath, 'r+', driver='mpio', comm=comm) as fObj:
        taucalc_bool = fObj[skew_key][taucalc_bool_key]
        curr_progress = np.sum(taucalc_bool) / taucalc_bool.size
        if verbose:
            print(f"--- {rank_idstr} : Starting calculations at {100 * curr_progress:.2f} % complete along ", Skewers_i.SkewersiHead.skew_key, "---")

        skewerID_arr = np.arange(Skewers_i.SkewersiHead.n_skews)
        ...
        for nSkewerID in skewerIDs_rank:
            ...
            densityHI = fObj[Skewers_i.SkewersiHead.skew_key].get('HI_density')[nSkewerID, :]
            temp = fObj[Skewers_i.SkewersiHead.skew_key].get('temperature')[nSkewerID, :]

            if nopec:
                taus = skewCosmoCalc.optical_depth_Hydrogen_nopec(densityHI, temp)
            else:
                vel = fObj[Skewers_i.SkewersiHead.skew_key].get('los_velocity')[nSkewerID, :]
    ...
    if verbose:
        print(f"--- {rank_idstr} : Effective optical depth calculation completed along ", Skewers_i.SkewersiHead.skew_key)
...
def main():
    ...
    Skewers = ChollaSkewers(skewer_fPath, comm)
    ...
    if args.verbose:
        print(f"--- {rank_idstr} : Cosmo Head and Skewer objects created ---")

    Skewers_x = Skewers.get_skewersx_obj()
    Skewers_y = Skewers.get_skewersy_obj()
    Skewers_z = Skewers.get_skewersz_obj()

    if args.verbose:
        print(f"--- {rank_idstr} : Skewers in each axis are created ---")

    skewCosmoCalc_x = ChollaSkewerCosmoCalculator(Skewers.current_a, chCosmoHead, Skewers.nx, Skewers.dx_h_kpc, precision)
    skewCosmoCalc_y = ChollaSkewerCosmoCalculator(Skewers.current_a, chCosmoHead, Skewers.ny, Skewers.dy_h_kpc, precision)
    skewCosmoCalc_z = ChollaSkewerCosmoCalculator(Skewers.current_a, chCosmoHead, Skewers.nz, Skewers.dz_h_kpc, precision)
    ...
    taucalc(Skewers_x, skewCosmoCalc_x, comm, precision, args.verbose, args.peculiarless)
    if args.verbose:
        t_x = MPI.Wtime()
