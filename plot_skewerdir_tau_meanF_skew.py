#!/usr/bin/env python3
"""
This script will plot the optical depth corresponding to the mean flux along 
    a skewer in a distribution as a function of redshift. It will take in a 
    skewer directory and a list of output strings and make a 2D histogram.
...
def create_parser():
    ...
    parser = argparse.ArgumentParser(
        description="Plot the distribution of optical depths of mean flux along a skewer")
...
def main():
    '''
    Plot the optical depth of the mean flux along a skewer as a function of redshift
    '''
    ...
    if args.fname:
        ...
        fName = f'tau_meanF_skewdistr.png'
    ...
    hist_meanF_ltau = np.zeros((tau_nbins-1, nOutputs))
    tau_meanF_mean_arr = np.zeros(nOutputs, dtype=precision)
    tau_meanF_18perc_arr = np.zeros(nOutputs, dtype=precision)
    tau_meanF_50perc_arr = np.zeros(nOutputs, dtype=precision)
    tau_meanF_84perc_arr = np.zeros(nOutputs, dtype=precision)
    ...
    for fPath in skewer_dirPath.iterdir():
        ...
        fluxes_x = np.exp(- tau_local_x)
        fluxes_y = np.exp(- tau_local_y)
        fluxes_z = np.exp(- tau_local_z)

        meanF_x = np.mean(fluxes_x, axis=1)
        meanF_y = np.mean(fluxes_y, axis=1)
        meanF_z = np.mean(fluxes_z, axis=1)

        tau_meanF_x = - np.log(meanF_x)
        tau_meanF_y = - np.log(meanF_y)
        tau_meanF_z = - np.log(meanF_z)

        nSkews_x = tau_meanF_x.size
        nSkews_y = tau_meanF_y.size
        nSkews_z = tau_meanF_z.size
        totnSkews = nSkews_x + nSkews_y + nSkews_z

        # place all optical depths into one array
        tau_meanF_all = np.zeros(totnSkews, dtype=precision)
        tau_meanF_all[ : (nSkews_x)] = tau_meanF_x.flatten()
        tau_meanF_all[ (nSkews_x) : (nSkews_x + nSkews_y)] = tau_meanF_y.flatten()
        tau_meanF_all[ (nSkews_x + nSkews_y) : ] = tau_meanF_z.flatten()

        # calculate mean, 16-50-84 percentiles
        tau_meanF_mean_arr[n] = np.mean(tau_meanF_all)
        tau_meanF_18perc_arr[n] = np.percentile(tau_meanF_all, 18)
        tau_meanF_50perc_arr[n] = np.percentile(tau_meanF_all, 50)
        tau_meanF_84perc_arr[n] = np.percentile(tau_meanF_all, 84)

        # histogram it !
        l_tau_hist_all, _ = np.histogram(np.log10(tau_meanF_all), bins=l_tau_bins)

        # place onto global array
        hist_meanF_ltau[:,n] += l_tau_hist_all

        # normalize at each redshift bin
        hist_meanF_ltau[:,n] = hist_meanF_ltau[:,n] / totnSkews
        n += 1
    
    indices_redshiftsort = np.argsort(redshift_bins)
    redshift_bins_sorted = redshift_bins[indices_redshiftsort]
    hist_meanF_ltau_sorted = hist_meanF_ltau[:, indices_redshiftsort[1:]] # first index will be zero
    tau_meanF_mean_arr_sorted = tau_meanF_mean_arr[indices_redshiftsort[1:]]
    tau_meanF_18perc_arr_sorted = tau_meanF_18perc_arr[indices_redshiftsort[1:]]
    tau_meanF_50perc_arr_sorted = tau_meanF_50perc_arr[indices_redshiftsort[1:]]
    tau_meanF_84perc_arr_sorted = tau_meanF_84perc_arr[indices_redshiftsort[1:]]

    redshift_center_sorted = (redshift_bins_sorted[1:] + redshift_bins_sorted[:-1]) / 2.


    yaxis_label = r'$-\ln(F_{\rm{skewer,mean}})$'
    ...
    cbar_str = r"$\log_{10} \rm{P}(-\ln (F_{\rm{skewer,mean}}) | z)$"
