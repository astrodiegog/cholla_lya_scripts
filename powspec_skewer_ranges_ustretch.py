...
└── ...
    ...
    Flux fluctuations are stretched to new max linear velocity bin

Usage for skewers bounded by optical depths 0.001 and 100.0 with linear velocity
    bin max of 10000.0:
    $ python3 powspec_skewer_range.py 0_skewers.h5 0.001 100.0 10000.0 -v
...
# ChollaFluxPowerSpectrumVelStretchHead --> holds rebinning info
class ChollaFluxPowerSpectrumVelStretchHead:
    '''
    Cholla Flux Power Spectrum - Velocity Stretch - Head

    Holds information regarding the power spectrum calculation where flux
        fluctuations are mirrord and evaluated in a stretched velocity space
    ...
    Values are returned in code units unless otherwise specified
    '''
    def __init__(self, ChollaFPSHead, u_max_stretch):
    ...
    def get_kvals_fft(self, dtype=np.float32):
        ...
        kcenters_fft = np.zeros(self.n_ustretch_fft, dtype=dtype)
        iter_arr = np.arange(self.n_ustretch_fft, dtype=dtype)

        kcenters_fft[:] = (2. * np.pi * iter_arr) / (self.ustretch_max)
    ...
    def get_FPS(self, local_opticaldepths, flux_mean_global=None, precision=np.float64):
        ...
        delta_F_avg = np.zeros(self.n_ustretch_fft , dtype=precision)
        P_k_tot = np.zeros(self.n_ustretch_fft, dtype=precision)

        for nSkewerID in range(n_skews):
            # calculate flux fluctuation 
            dFlux_skew = (fluxes[nSkewerID] - flux_mean) / flux_mean

            dFlux_skew_stretch = dFlux_skew[self.iter_arr_2stretch]

            # perform fft & calculate amplitude of fft
            fft = np.fft.rfft(dFlux_skew_stretch)
            fft2 = (fft.imag * fft.imag) + (fft.real * fft.real)

            # take avg & scale by umax
            delta_F_avg = fft2 / self.n_ustretch_los / self.n_ustretch_los
            P_k = self.ustretch_max * delta_F_avg
            P_k_tot += P_k
...
def main():
    ...
    # make sure u_max makes sense
    assert args.u_max_stretch > 0.
    if args.verbose:
        print(f'--- Stretching maximum velocity scale to {args.u_max_stretch:.3e}---')
    ...
    outfile_fname = f"{nOutput:.0f}_fluxpowerspectrum_ustretch_optdepthbin.h5"
    ...
    if args.verbose:
        ...
        if outfile_exists:
            ...
            print(calc_string)
        if args.nonstretched:
            print(f'--- Calculating + saving FPS without stretching ---')
        else:
            print(f'--- Not calculating the original / un-stretched FPS---')
    ...
    mean_flux_local_inbounds = np.mean(fluxes_local_inbounds)
    mean_flux_eff_inbounds = np.mean(fluxes_eff_inbounds)
    med_flux_local_inbounds = np.percentile(fluxes_local_inbounds, 50)
    med_flux_eff_inbounds = np.percentile(fluxes_eff_inbounds, 50)

    # calculate effective optical depth wrt mean flux values
    tau_mean_flux_local_inbounds = -np.log(mean_flux_local_inbounds)
    tau_mean_flux_eff_inbounds = -np.log(mean_flux_eff_inbounds)
    tau_med_flux_local_inbounds = -np.log(med_flux_local_inbounds)
    tau_med_flux_eff_inbounds = -np.log(med_flux_eff_inbounds)

    if args.verbose:
        curr_str = f'--- Mean local flux (inbounds) : {mean_flux_local_inbounds:.4e} / tau : {tau_mean_flux_local_inbounds:.4e} ---'
        curr_str = f'--- Mean effective flux (inbounds) : {mean_flux_eff_inbounds:.4e} / tau : {tau_mean_flux_eff_inbounds:.4e} ---'
        print(curr_str)
    ...
    FPS_ustretch_y = np.zeros(FPSuStretchHead_y.n_ustretch_fft, dtype=precision)
    FPS_ustretch_z = np.zeros(FPSuStretchHead_z.n_ustretch_fft, dtype=precision)
    if args.nonstretched:
    ...
    if not outfile_exists:
        # calculate kmodes
        kvals_ustretch_fft = FPSuStretchHead_x.get_kvals_fft(dtype=precision)
    ...
    if nskews_x_inbounds:
        ...
        _, calc_FPS_x = FPSuStretchHead_x.get_FPS(tau_local_x_inbounds,
                                                flux_mean_global=med_flux_eff_inbounds,
                                                precision=precision)
        FPS_ustretch_x += calc_FPS_x
    ...
    if nskews_y_inbounds:
        ...
        _, calc_FPS_y = FPSuStretchHead_y.get_FPS(tau_local_y_inbounds,
                                          flux_mean_global=med_flux_eff_inbounds,
                                          precision=precision)
        FPS_ustretch_y += calc_FPS_y
    ...
    if nskews_z_inbounds:    
        ...
        _, calc_FPS_z = FPSuStretchHead_z.get_FPS(tau_local_z_inbounds,
                                          flux_mean_global=med_flux_eff_inbounds,
                                          precision=precision)
        FPS_ustretch_z += calc_FPS_z
    ...
    with h5py.File(outfile_fPath, 'a') as fObj:
        ...
        if not outfile_exists:
            ...
            _ = fObj.create_dataset('k_ustretch', data=kvals_ustretch_fft)
        ...
        _ = range_group.attrs.create('mean_flux_local_inrange', mean_flux_local_inbounds)
        _ = range_group.attrs.create('mean_flux_eff_inrange', mean_flux_eff_inbounds)

        _ = range_group.attrs.create('tau_mean_flux_local_inrange', tau_mean_flux_local_inbounds)
        ...
        _ = range_group.create_dataset('FPS_ustretch_x', data=FPS_ustretch_x)
        _ = range_group.create_dataset('FPS_ustretch_y', data=FPS_ustretch_y)
        _ = range_group.create_dataset('FPS_ustretch_z', data=FPS_ustretch_z)
