...
def create_parser():
    ...
    parser.add_argument('-t', '--total1D', help='Complete rFFT all at once rather than looping per skewer', 
                            action='store_true')
...
def main():
    ...
    if args.total1D:
        dFlux_tot = chFPSHead.get_flux_overdensity(tau_loc_tot, precision=precision)
        fft = np.fft.rfft(dFlux_tot, axis=1) 
        fft2 = (fft.imag * fft.imag) + (fft.real * fft.real)
        delta_F_all = fft2 / chFPSHead.n_los / chFPSHead.n_los
        PF_k_tot[:] = chFPSHead.u_max * delta_F_all
    else:
        for skewID in range(nSkews_tot):
            tau_loc = tau_loc_tot[skewID, :]
            PF_k = chFPSHead.get_flux_power_spectra(tau_loc, precision=precision)
            PF_k_tot[skewID, :] = PF_k
